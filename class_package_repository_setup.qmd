...
Here's an except from "R Packages" by Hadley Wickham and Jennifer Bryan
regarding a new package named `regexcite`. We'll end up adding some more
directories to the package for some other file types.
...
>     you aren't using
>     RStudio, [`create_package()`](https://usethis.r-lib.org/reference/create_package.html) may
>     not create this file (nor `.gitignore`) at first, since there's no
>     RStudio-related machinery that needs to be ignored. However, you
>     will likely develop the need for `.Rbuildignore` at some point,
>     regardless of what editor you are using. It is discussed in more
>     detail in [Section
...
>     shortly and [Chapter
>     9](https://r-pkgs.org/description.html) covers the general topic
...
>     you can suppress its creation
>     with `create_package(..., rstudio = FALSE)`.
...
we need to set a name for Git to know us by. See [this
article](https://docs.github.com/en/get-started/getting-started-with-git/setting-your-username-in-git)
for more details. Open terminal and enter one of the following depending
on whether you want to set a name for this repository or for all
repositories on your computer. This username DOES NOT NEED TO MATCH YOUR
GITHUB USERNAME.

For this repository, run the following in the terminal:

    git config user.name "yournamehere"

For all repositories on your computer, run the following in the
terminal:

    git config --global user.name "yournamehere"

We'll need to do this for user.email too, but THIS EMAIL MUST MATCH ONE
OF THE ONES ASSOCIATED WITH YOUR GITHUB ACCOUNT.
