...
> within [`select()`](https://dplyr.tidyverse.org/reference/select.html):
>
...
if you don't want to remove all the other columns, run the same thing as above but use rename instead of select
...
we can add .before or .after along with a column name to specify that we want to move the column(s) to a specific place
